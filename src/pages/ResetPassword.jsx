...
const ResetPassword = () => {
   ...
   const [otp,setOtp] = useState("");
    ...
    const onSubmitEmail = async (e) => {
        e.preventDefault();
        try {
           const response = await axios.post(backendURL + "/reset-otp?email=" + email);
    ...
    const onSubmitNewPassword = async (e) => {
        e.preventDefault();
        try {
           let response = await axios.post(backendURL+"/reset-password",{email,otp,newPassword});
           if(response.status == 200){
    ...
    return (
        ...
        style={{background:"linear-gradient(90deg,#6a5af9,#8268f9)",border:"none"}}>
                ...
                <div className="rounded-4 p-5 text-center bg-white" style={{width:'100%',maxWidth:"400px"}}>
                    ...
                    <form onSubmit={onSubmitEmail}>
                        <div className="input-group mb-4 bg-secondary bg-opacity-10 rounded-pill">
                            ...
                            <input type="email" className="form-control bg-transparent border-0 ps-1 pe-4 rounded-end" placeholder="Enter your email address"
                            style={{height:'50px'}}
            ...
            {isOtpSubmitted && isEmailSent && (
                <div className="rounded-4 p-4 text-center bg-white" style={
                    ...
                    <form onSubmit={onSubmitNewPassword}>
                        <div className="input-group mb-4 bg-secondary bg-opacity-10 rounded-pill">
                            ...
                            <input type="password" className=
                                "form-control bg-transparent border-0 ps-1 pe-4 rounded-end" placeholder="*************"
                                   style={{height:'50px'}} onChange={e=>setNewPassword(e.target.value)}
